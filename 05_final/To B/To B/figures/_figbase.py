# -*- coding: utf-8 -*-
"""P2 数据图公共引导模块 —— 视觉语言与 P1（problem1/05_final/To B）绘图 session 对齐。

统一提供：
  * 工作区锚定（os.chdir(ROOT)，保证 setup_style 种子稳定）；
  * 裸调 setup_style()（同工作区名 "To B" → 同 P1 的 tol_vibrant 配色）；
  ...
  * 固定视觉编码（P1 同名变量取同色）、字号常量、数据读取、绘图前数值校验。
...
# 使配色不再随工作区目录名变化（P1 energy/storage 同款）。
...
# 固定视觉编码（与 P1 energy/storage 一致：tol_vibrant）
C_PRIMARY = PALETTE[0]      # 蓝  #0077BB —— 计划购电 / 主策略 / LightGBM
C_ORANGE = PALETTE[1]       # 橙  #EE7733 —— 第四类对象（P1 energy 未用，同属 tol_vibrant）
C_TEAL = PALETTE[2]         # 青  #009988 —— 电价 / 次要对照（P1 energy 电价、storage 放电）
C_RED = PALETTE[3]          # 红  #CC3311 —— 紧急购电 / 风险 / PV / SOC（P1 energy PV、storage SOC）
