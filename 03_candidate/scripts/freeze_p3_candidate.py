...
STATE_OUTPUTS = {'CANDIDATE_FREEZE_MANIFEST.json','CANDIDATE_MANIFEST_SHA256.txt',
                 'STATE_UPDATE_RECORD.json'}
AUTHORIZED_JUNCTION = 'A_route/problem3/02_batch_run/scripts/node_modules'
AUTHORIZED_JUNCTION_TARGET = 'C:/Users/JINPU/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/node_modules'
...
def junction_record(root: Path) -> dict:
    """只登记用户授权的唯一目录联接，不读取其外部子树。"""
    path=root/AUTHORIZED_JUNCTION
    contract=read_json(root/'A_route/problem3/03_candidate/contracts/P3_CANDIDATE_JUNCTION_EXCEPTION.json')
    if contract['path']!=AUTHORIZED_JUNCTION or contract['resolved_target']!=AUTHORIZED_JUNCTION_TARGET:
        raise FreezeError('目录联接例外合同超出授权范围')
    if path.is_symlink() or not path.is_junction() or path.resolve()!=Path(AUTHORIZED_JUNCTION_TARGET).resolve():
        raise FreezeError('授权目录联接缺失、类型或目标改变：'+str(path))
    st=path.lstat()
    return {'path':AUTHORIZED_JUNCTION,'resolved_target':path.resolve().as_posix(),
            'raw_target':os.readlink(path),'kind':'项目外部运行时依赖目录联接',
            'reason':contract['reason'],'recursive_entry':False,
            'device':st.st_dev,'inode':st.st_ino,'mtime_ns':st.st_mtime_ns,
            'file_attributes':st.st_file_attributes,'reparse_tag':st.st_reparse_tag}


def frozen_tree_paths(folder: Path, root: Path):
    """遍历项目内原件；仅在递归前跳过精确授权联接，其他联接不豁免。"""
    for path in sorted(folder.iterdir()):
        if path==root/AUTHORIZED_JUNCTION:
            junction_record(root)
            continue
        if path.is_symlink() or path.is_junction():
            raise FreezeError('未授权的联接或符号链接：'+str(path))
        safe_join(root,path.relative_to(root).as_posix())
        yield path
        if path.is_dir():
            yield from frozen_tree_paths(path,root)


def inventory_tree(folder: Path, root: Path, role: str) -> list[dict]:
    records=[]
    for path in sorted(frozen_tree_paths(folder,root)):
...
def verify_manifest(root: Path, cand: Path):
    ...
    if manifest.get('external_runtime_junction')!=junction_record(root):bad.append('授权目录联接身份改变')
    ...
    current={p.relative_to(root).as_posix() for p in frozen_tree_paths(batch,root) if p.is_file() and not excluded(p)}
...
def write_outputs(root: Path, batch: Path, cand: Path, spec: dict, data: dict,
                  check: Checks, before: list[dict], state_before: str | None, junction_before: dict):
    ...
    current_paths={p.relative_to(root).as_posix() for p in frozen_tree_paths(batch,root) if p.is_file() and not excluded(p)}
    check.add('BATCH_FILE_SET_UNCHANGED',current_paths=={r['path'] for r in before if r['role']=='batch_canonical_or_evidence'})
    check.add('AUTHORIZED_JUNCTION_UNCHANGED',junction_before==junction_record(root))
    ...
    manifest={'schema':'P3_CANDIDATE_FREEZE_MANIFEST_V1','gate':PASS_GATE,'candidate_id':MAIN,
              'external_runtime_junction':junction_before,
...
def main():
    ...
    try:
        ...
        junction_before=junction_record(root)
        check.add('SINGLE_AUTHORIZED_JUNCTION_REGISTERED',True,AUTHORIZED_JUNCTION)
        ...
        result=write_outputs(root,batch,cand,spec,data,check,before,state_before,junction_before)
        ...
        if not args.verify_only and not (cand/'CANDIDATE_FREEZE_MANIFEST.json').exists():
            failure=cand/'FREEZE_FAILURE.json'
            if failure.exists():failure=cand/('FREEZE_FAILURE_'+uuid.uuid4().hex+'.json')
